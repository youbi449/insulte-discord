...
if (!token || !clientId) {
  logError('Variables d\'environnement manquantes. Assurez-vous d\'avoir un fichier .env avec DISCORD_TOKEN et CLIENT_ID.');
  process.exit(1);
}
...
for (const file of commandFiles) {
  ...
  if ('data' in command && 'execute' in command) {
    ...
    console.warn(`⚠️ La commande ${filePath} ne contient pas les propriétés "data" ou "execute" requises.`);
  }
}
...
async function deployCommands() {
  try {
    ...
    if (guildId) {
      // Déploiement sur un serveur spécifique (test)
      data = await rest.put(
